/**
 * @file care_safety_controller_node.cpp
 * @brief C.A.R.E. Safety Controller Node - Централизованная система принятия решений о безопасности
 * @author C.A.R.E. Development Team
 * @date 2025-10-01
 * @version 2.0.0
 *
 * @details Этот узел является центральным компонентом системы безопасности C.A.R.E.
 * Он анализирует данные со всех источников (радары, демо-симуляция) и принимает
 * решения об активации защитных мер (замедление, экстренная остановка).
 *
 * ## Основные функции:
 * - Получение данных от всех источников (CAN, Demo, UART)
 * - Анализ зон безопасности (двухуровневая система: замедление + остановка)
 * - Принятие решений об экстренной остановке с гистерезисом
 * - Отправка команд безопасности всем контроллерам
 * - Ведение подробных логов событий безопасности
 *
 * ## ROS2 Интерфейсы:
 *
 * ### Подписки:
 * - `/care/device_{1..15}/targets` (RadarTargets) - данные с физических радаров
 * - `/care/demo/targets` (RadarTargets) - данные из демо-симуляции
 *
 * ### Публикации:
 * - `/care/safety_commands` (SafetyCommand) - команды безопасности для контроллеров
 * - `/care/system_status` (SystemStatus) - статус системы (1 Hz)
 *
 * ### Сервисы:
 * - `/care/set_safety_zone` (SetSafetyZone) - настройка зон безопасности
 * - `/care/emergency_stop` (EmergencyStop) - ручная активация/деактивация экстренной остановки
 * - `/care/get_system_status` (GetSystemStatus) - получение детального статуса системы
 *
 * ## Параметры:
 * - `max_devices` (int, default: 15) - максимальное количество подключенных устройств
 * - `safety_check_rate` (double, default: 20.0) - частота проверки безопасности (Hz)
 * - `default_safety_distance` (double, default: 300.0) - дистанция критической зоны (мм)
 * - `default_safety_angle` (double, default: 60.0) - угол зоны безопасности (градусы)
 * - `emergency_hysteresis` (double, default: 50.0) - гистерезис для снятия экстренной остановки (мм)
 * - `min_trigger_time` (double, default: 0.1) - минимальное время для активации (сек)
 * - `log_events` (bool, default: true) - включить логирование событий
 * - `log_file` (string, default: "/tmp/care_safety.log") - путь к файлу логов
 *
 * ## Алгоритм двухуровневой защиты:
 * 1. **Зона замедления** (slowdown_distance): Цель обнаружена на среднем расстоянии →
 *    отправляется COMMAND_SLOWDOWN для снижения скорости
 * 2. **Критическая зона** (critical_distance): Цель слишком близко →
 *    отправляется COMMAND_STOP для немедленной остановки
 * 3. **Гистерезис**: После снятия угрозы система ждёт, пока все цели не выйдут на
 *    расстояние critical_distance + emergency_hysteresis, чтобы избежать дребезга
 *
 * ## Пример запуска:
 * @code
 * ros2 run care_safety_controller_node care_safety_controller_node \
 *   --ros-args \
 *   -p safety_check_rate:=50.0 \
 *   -p default_safety_distance:=500.0 \
 *   -p log_file:=/var/log/care_safety.log
 * @endcode
 */
